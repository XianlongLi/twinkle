"""GRPO training with routing replay for MoE models (Ray direct mode).
...
Set ROUTER_REPLAY_MODE environment variable to select the mode.

Usage:
    # Disabled (default)
    python cookbook/rl/grpo_routing_replay.py

    # R2 mode
    ROUTER_REPLAY_MODE=R2 python cookbook/rl/grpo_routing_replay.py

    # R3 mode
    ROUTER_REPLAY_MODE=R3 python cookbook/rl/grpo_routing_replay.py
"""
import os
import re
...
if ROUTER_REPLAY_MODE != 'disabled' and not USE_MEGATRON:
    raise ValueError('Routing replay requires USE_MEGATRON=1 (Megatron backend)')
        ...
        for mb_start in range(0, total_completions, MINI_BATCH_SIZE):
            ...
            if ROUTER_REPLAY_MODE == 'R2':
                ...
                if routed_experts is not None:
                    # Split [1, total_seq, L, K] by per-sequence length and
                    # inject into each InputFeature so the processor collates
                    # them together in forward_backward.
                    offset = 0
                    for inp in mb_inputs:
